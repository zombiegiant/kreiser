...
void setup() {
  ...
  SPI.begin();
  Ethernet.begin(mac);// Init SPI bus
...
void loop() {
 ...
 data.concat(uidDec);
   ...
   if (client.connect("80.87.195.174",80)) 
  {   
    client.println("POST /novros/payment_card.php HTTP/1.1"); 
...
while(client.connected()) 
{
   if(client.available())
   {
      ...
      String number = c.substring(c.length()-2,c.length());
        lcd.print(number);
